...
{literal}
<video id="my_video_1" class="video-js {/literal}{$VIDEOJS_SKIN}{literal}" width={/literal}"{$WIDTH}"{literal} height={/literal}"{$HEIGHT}"{literal} poster={/literal}"{$VIDEOJS_POSTER_URL}"{literal} data-setup='{controls":{/literal}{$CONTROLS}{literal}, "preload": {/literal}"{$PRELOAD}"{literal}, "autoplay": {/literal}{$AUTOPLAY}{literal}, loop: {/literal{$LOOP}{literal} }'>
